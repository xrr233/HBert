...
def get_layer_info(nodes,max_layer_num,node_max_length):
    node_queue=[]
    nodes_info = []
    attention_mask = []
    nodes[0]["depth"] = 1
    ...
    while(len(node_queue)>0):
        ...
        if node_queue[0]["name"] == "textnode" or node_queue[0]["depth"] == max_layer_num :
            ...
            layer_index.append(node_queue[0]["depth"])
            waiting_mask = [0]*node_max_length
            ...
            type_idx=[1]
            layer_index.append(node_queue[0]["depth"])
            waiting_mask = [0]+[1]*children_num + [0] * max(node_max_length-(children_num+1),0)
    ...
    return nodes_info,layer_index,waiting_masks
...
def construct_pointwise_examples(examples,chunk_indexs,max_seq_len,max_tag_len,max_layer_num,mlm,bert_tokenizer,masked_lm_prob,
    ...
    for doc_idx in tqdm(chunk_indexs):
        ...
        nodes_info,layer_index,waiting_mask = get_layer_info(example,max_layer_num=max_layer_num,node_max_length=max_seq_len)
        tag_len = len(layer_index)

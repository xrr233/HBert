...
class HirachicalBert(FraBert):
    ...
    def __init__(self, text_config, node_config, layer_num, num_type=3):
        ...
        self.node_bert = nn.ModuleList([BertModel(node_config) for i in range(layer_num)])
    ...
    def get_task_embeddings(self, embeddings, negetive_embeddings, mask_embeddings,waiting_mask,node_layer_index):
        ...
        for i in range(batch_size):
            for j in range(tag_len):
                ...
                if waiting_mask[i,j,temp[1]]:
                    brother.append(embeddings[i,j,temp[1]])
        ...
        node_layer = node_layer_index[candidates_mask]
        ...
        return train_embeddings, negetive_embeddings,torch.stack(brother),node_layer,tag_labels.bool()
    ...
    def forward(
            self,
            node_layer_index,#(batch_size,node_num)
            token_layer_index,
            position,
            waiting_mask,
        ...
        for layer in range(self.layer_num):
            ...
            if len(torch.nonzero(node_layer_mask, as_tuple=False)):
                ...
                layer_node_outputs = self.node_bert[layer](
                    input_ids=None,
                    attention_mask=None,
                    token_type_ids=None,
                    position_ids=None,
        ...
        train_embeddings,negetive_embeddings,brother,node_layer,task1_labels = self.get_task_embeddings(all_embeddings,neg_embeddings,mask_embeddings,waiting_mask,node_layer_index)
        cls_len = int(len(negetive_embeddings))
        original = train_embeddings[:cls_len]
        mask = train_embeddings[cls_len:]
        orginal_output = None
        mask_output = None
        for i in range(self.layer_num):
            cur = self.layer_num-i
            layer_mask = node_layer.eq(cur)
            layer_original_embeds = original[layer_mask]
            layer_mask_embeds = mask[layer_mask]
            if len(layer_original_embeds):
                layer_original_output = self.node_bert[i](
                        input_ids=None,
                        attention_mask=None,
                        token_type_ids=None,
                        position_ids=None,
                        head_mask=None,
                        inputs_embeds=layer_original_embeds,
                    )[0]
                if orginal_output == None:
                    orginal_output = layer_original_output
                else:
                    orginal_output = torch.cat((layer_original_output,orginal_output),dim=0)
            if len(layer_mask_embeds):
                layer_mask_output = self.node_bert[i](
                    input_ids=None,
                    attention_mask=None,
                    token_type_ids=None,
                    position_ids=None,
                    head_mask=None,
                    inputs_embeds=layer_mask_embeds,
                )[0]
                if mask_output == None:
                    mask_output = layer_mask_output
                else:
                    mask_output = torch.cat((layer_mask_output, mask_output), dim=0)




        device = node_output.device
        positive = orginal_output[task1_labels]
        #print(mask.size())
        #pc_parent = mask[:,0,:]
        #pos_score = self.masked_item_prediction(brother,pc_positive).view(1,-1)
        #neg_score = self.masked_item_prediction(brother,pc_negetive).view(1,-1)

...
class HirachicalBert(FraBert):
    ...
    def get_embedding(self, output, waiting_mask, node_input_ids):
        ...
        pos_embeddings = self.bert.embeddings.word_embeddings(node_input_ids)
        ...
        pos_embeddings[waiting_mask.type(torch.bool)] = res
       # pos_embedds = pos_embeddings.clone().detach()
        return all_embeddings, pos_embeddings,neg_embeddings


    def get_task_embeddings(self, embeddings,positive_embeddings ,negetive_embeddings, waiting_mask):
            ...
            for j in range(tag_len):
                ...
                for k in range(mask_num):
                    if waiting_mask[i, j, temp[k]]:
                        ...
                        embeddings[i][j][temp[k]] = self.bert.embeddings.word_embeddings(torch.tensor(103).to(device))
        ...
        positive_embeddings = positive_embeddings[candidates_mask]
        ...
        return embeddings,positive_embeddings,negetive_embeddings, tag_labels.bool()
    ...
    def sample_one_embedding(self, waiting_mask, embeddings, i, j):
        ...
        # else :
        #     return 0
    def masked_item_prediction(self,sequence_output,target_item,task1_labels):
    ...
    def forward(
        ...
        all_embeddings,positive_embeddings,neg_embeddings = self.get_embedding(output,waiting_mask,node_input_ids)
        train_embeddings,positive_embeddings,neg_embeddings,task1_labels = self.get_task_embeddings(all_embeddings,positive_embeddings,neg_embeddings,waiting_mask)
        ...
        pos_score = self.masked_item_prediction(node_output, positive_embeddings,task1_labels).view(1,-1)
        neg_score = self.masked_item_prediction(node_output, neg_embeddings,task1_labels).view(1,-1)
        scores = torch.cat((pos_score,neg_score),dim=0).permute(1,0)


        mip_loss = criterion(scores, torch.ones(pos_score.size(-1)).to(device).long())
        task1_loss = torch.sum(mip_loss * task1_labels.flatten())/len(torch.nonzero(task1_labels,as_tuple=False))
        #print("loss:",task1_loss)
        cls_len = int(len(node_output) / 2)
        # positive = node_output[:cls_len]
        # positive = positive[task1_labels]
        # negetive = node_output[cls_len:]
        # negetive = negetive[task1_labels]
        #cls = node_output[:,0,:].view(-1,self.config.hidden_size)
        #cls_logits = self.linear(cls)
        # cls_score = cls_logits[:,1]
        #task1_loss = task1_loss_fct(positive,negetive,torch.ones(len(positive)).to(device))
        ...
        #print(text_mlm_loss,task1_loss.double())
        #print(task1_loss)
        return {
